...
class Imdb:
    def __init__(self):
        self.imdb_files = ["title.basics.tsv", "title.ratings.tsv"]
                           #, "name.basics.tsv", "title.akas.tsv", "title.crew.tsv", "title.episode.tsv", "title.principals.tsv"]
        self.db = Db()
        self.files = {}
    
    def download_files(self):
        for imdb_file in self.imdb_files:
            self.files[imdb_file] = Utils.download_file(f"https://datasets.imdbws.com/{imdb_file}.gz")

    def get_df(self, table_name):
        dl_file = self.files[f'{table_name}.tsv']
        log.info(f"read tsv: {table_name}")
        with gzip.open(dl_file, 'rb') as file:
            df = pandas.read_csv(dl_file, sep="\t", low_memory=False, na_values="\\N")
        df.columns = [Utils.camel_to_snake(c) for c in df.columns]
        return df
    ...
    def title_basics(self):
        ...
        self.db.df2pg(df, f"imdb.{table_name.replace('.','_')}")
        del df
